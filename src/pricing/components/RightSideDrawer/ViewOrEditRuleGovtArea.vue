  ...
  <div>
    <v-container class="px-12">
      <h1 class="primary--text font-weight-bold pb-4">
        {{
          isEditPlace
            ? $t("places.editGovernorateArea")
            : $t("places.viewGovernorateArea")
        }}
      </h1>
...
<script>
import { updateGovernorateAreaTransferPrice } from "@/pricing/methods/update_govt_to_area.js";
...
export default {
  data() {
    return {
      placesInput: {
        ...
        from: this.currentPlace.governorate._id,
        to: this.currentPlace.area._id,
  ...
  methods: {
    editThisProduct() {
      ...
      updateGovernorateAreaTransferPrice(this.placesInput).then((data) => {
        if (!!data.data) {
